...
it('Validating all the deals on home page - Normal way USING get and its', function()
{
 ...
 cy.get('.deals__title:visible').contains('Today\'s Flight Deals').and('not.be.disabled')       
 ...
 cy.get('.deals__title:visible').contains('Top Car Deals').and('not.be.disabled')


 // Using eq() indexing 
 cy.get('.deals__title:visible').eq(4).contains('Top Car Deals').and('not.be.disabled')
...
it('Validating all the deals on home page - Using multiple assertion', function()
{
    ...
    let deal_list = ['Today\'s Flight Deals*','Featured Offers*','Our Best Price*','Top Hotel Deals*','Top Car Deals*'];
